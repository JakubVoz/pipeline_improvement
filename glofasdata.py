...
class GlofasData:
    ...
    def zonalStatGlofasDataGridToCsv(self):
        ...
        for ens in range(0, self.nofEns):
            ...
            for i in range(0, 7):
                ...
                df_leadtime_ens.append(df_dis.filter(
                    ['pcode', 'placeCode', 'placeCodeParent', 'ensemble', 'leadTime', 'dis', 'rl2', 'rl5', 'rl20']))
    ...
    def pointGlofasDataGridToCsv(self):
        ...
        stations = (df_district_mapping.query('placeCode == @extractAdminCode')['glofasStation'].values[0] for
                    extractAdminCode in selectedPcodeVal)
        ...
        for ens in range(0, self.nofEns):
            ...
            for i in range(0, 7):
                ...
                df_dis['leadTime'] = leadTimelabel
                df_leadtime_ens.append(df_dis.filter(
                    ['pcode', 'station', 'placeCode', 'placeCodeParent', 'ensemble', 'leadTime', 'dis', 'rl2', 'rl5',
                     'rl20']))
            glofasDffinal = pd.concat(df_leadtime_ens)
    ...
    def extractGlofasDataGrid(self):
        ...
        for extractAdminCode in selectedPcodeVal:
            ...
            if station['code'] in df_thresholds[
                'stationCode']:  # and station['code'] in df_district_mapping['glofasStation']:
                ...
                for step in range(1, 8):
                    ...
                    for ensemble in range(0, ensemble_options):
                        discharge = \
                        glofasDffinal.query('pcode in @extractAdminCode').query('ensemble ==@ensemble').query(
                            'leadTime ==@leadTimelabel')['dis'].values[0]
    ...
    def extractGlofasData(self):
        ...
        if self.countryCodeISO3 == 'PHL':
            ...
            dffinal = pd.merge(df.copy(), dfrp.filter(['stationCode', 'lat', 'lon', '2y', '5y', '20y', 'plat', 'plon']),
                               how='left', left_on='stationCode', right_on='stationCode')
            ...
            if station['code'] in df_district_mapping['glofasStation']:
                ...
                for step in range(1, 8):
                    ...
                    if not data.empty:
                        ...
                        result = data.agg(
                            {'dis': ['sum', 'count'], 'thresholdCheck': ['sum', 'count'], 'member': ['sum', 'count']})
                        ...
                        station['fc_prob'] = prob
                        station['fc_trigger'] = 1 if prob > self.TRIGGER_LEVELS['minimum'] else 0
                        station['eapAlertClass'] = self.checkTriggerProb(
                            prob)  # 1 if prob > self.eapAlertClass['min'] else 0
